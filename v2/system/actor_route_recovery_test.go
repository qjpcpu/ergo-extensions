...
func TestActorRouteOwnerFollowsNodeMembership(t *testing.T) {
	s := routeStore(t)
	r := routeRouter(t, s, ActorRouterOptions{})
	n := routeNode(t)
	r.Bind(n)
...
func TestActorRouteDiscoveryFailurePreventsTakeover(t *testing.T) {
	...
	for _, network := range []gen.Network{nil, routeFailNetwork{err: want}, routeFailNetwork{registrar: routeFailRegistrar{err: want}}} {
		...
		if _, _, e := r.lookup(nil, "key"); e == nil {
			t.Fatal("lookup hid discovery failure")
		}
