...
func TestCronTickPendingAndMissingBackendBranches(t *testing.T) {
	resolution := time.Minute
	now := time.Date(2026, 9, 2, 10, 5, 30, 0, time.UTC)
	actor := spawnCronUnit(t, nil)
	process := actor.Behavior().(*Process)
	process.options = SchedulerOptions{
		TickResolution:     resolution,
		LeaseTTL:           time.Minute,
		MaxDispatchPerTick: 1,
	}.WithDefaults()
	process.owned = make(map[uint32]*shardRuntime)
	if err := process.handleTick(now); err != nil {
		t.Fatalf("empty tick failed: %v", err)
	}
	if want := now.Truncate(resolution); !process.lastTick.Equal(want) {
		t.Fatalf("expected last tick %s, got %s", want, process.lastTick)
	}

	runtime := newShardRuntime(1, 1)
	runtime.loadedAt = now.Add(-3 * resolution)
	process.owned[1] = runtime
	if got := process.bootstrapLastTick(now); !got.Equal(runtime.loadedAt.Truncate(resolution).Add(-resolution)) {
		t.Fatalf("unexpected bootstrap tick: %s", got)
	}

	job, err := compileJob(JobSpec{
		ID:             "job-a",
		Schedule:       "* * * * *",
		TriggerProcess: "worker",
	})
	if err != nil {
		t.Fatal(err)
	}
	process.pending = []pendingDispatch{
		{job: job, shard: 1, dispatchKey: "a"},
		{job: job, shard: 1, dispatchKey: "b"},
	}
	process.flushPending()
	if len(process.pending) != 2 {
		t.Fatalf("missing backend should preserve pending work, got %d", len(process.pending))
	}
	if got := process.inspect(); got["owned_shards"] != "1" || got["pending_jobs"] != "2" {
		t.Fatalf("unexpected inspect result: %#v", got)
	}

	runtime.lease.ExpiresAt = time.Now().Add(-time.Second)
	if err := process.refreshShardLeases(); err == nil || !strings.Contains(err.Error(), "missing KV store") {
		t.Fatalf("expected missing backend error, got %v", err)
	}
	if err := process.replayShard(runtime, 2, 1); err != nil {
		t.Fatalf("empty replay failed: %v", err)
	}
	if err := process.collectShardSlot(runtime, now, nil); err == nil {
		t.Fatal("expected empty slot checkpoint to require a backend")
	}
	if err := process.collectShardSlot(runtime, now, []*CompiledJob{job}); err == nil {
		t.Fatal("expected claimed jobs to require a backend")
	}
}
