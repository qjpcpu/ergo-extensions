...
func TestCompiledScheduleNextCalendarAndDST(t *testing.T) {
	for _, test := range []struct{ name, spec, location, after, want string }{
		{"annual", "0 0 1 1 *", "UTC", "2026-01-01T00:00:00Z", "2027-01-01T00:00:00Z"},
		{"leap day", "0 0 29 2 *", "UTC", "2025-03-01T00:00:00Z", "2028-02-29T00:00:00Z"},
		{"short month", "0 0 31 * *", "UTC", "2026-04-01T00:00:00Z", "2026-05-31T00:00:00Z"},
		{"impossible day", "0 0 30 2 *", "UTC", "2026-01-01T00:00:00Z", ""},
		{"spring gap", "30 2 * * *", "America/New_York", "2026-03-08T06:59:00Z", "2026-03-09T06:30:00Z"},
		{"fall repeated minute", "30 1 * * *", "America/New_York", "2026-11-01T05:30:00Z", "2026-11-01T06:30:00Z"},
		{"fall next hour", "0 2 * * *", "America/New_York", "2026-11-01T05:30:00Z", "2026-11-01T07:00:00Z"},
		{"half hour spring gap", "45 2 * * *", "Australia/Lord_Howe", "2026-10-03T15:29:00Z", "2026-10-03T15:45:00Z"},
		{"half hour repeated minute", "45 1 * * *", "Australia/Lord_Howe", "2026-04-04T14:45:00Z", "2026-04-04T15:15:00Z"},
	} {
		t.Run(test.name, func(t *testing.T) {
			schedule, err := compileSchedule(test.spec, test.location)
			if err != nil {
				t.Fatal(err)
			}
			...
			if !ok || !next.Equal(want) {
				t.Fatalf("next occurrence: got %s (%v) want %s", next, ok, want)
			}
		})
	}
}

func BenchmarkCompiledScheduleNextAnnual(b *testing.B) {
	schedule, err := compileSchedule("0 0 1 1 *", "UTC")
	if err != nil {
		b.Fatal(err)
	}
