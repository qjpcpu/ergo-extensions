...
func (p *Process) commitShardRuntime(runtime *shardRuntime) error {
	...
	// Replay mutates the actor's pending queue, so commit it on the actor goroutine.
	currentSlot := slotKey(runtime.loadedAt, p.options.TickResolution)
...
func loadShardResults(shards []uint32, concurrency int, fn func(uint32) shardLoadResult) []shardLoadResult {
	...
	for range concurrency {
		...
		go func() {
			defer wg.Done()
			for job := range jobs {
				results[job.index] = fn(job.shard)
			}
		}()
	}
	for index, shard := range shards {
		jobs <- shardJob{index: index, shard: shard}
	}
	close(jobs)
	wg.Wait()
	return results
...
func (p *Process) collectShardSlot(runtime *shardRuntime, slotTime time.Time, jobs []*CompiledJob) error {
	...
	for _, record := range records {
		...
		// The next occurrence must remain scheduled while this dispatch is retried.
